...
namespace EditorHooks
{
    ...
    template<class T> class HookFunctor : public HookFunctorBase
    {
        public:
            ...
            { ; }
            virtual void Call(cbEditor* editor, wxStyledTextEvent& event) const
            {
                if (m_pObj && m_pFunc)
    ...
    class cbSmartIndentEditorHookFunctor : public HookFunctorBase
    {
        public:
            ...
            /** Needs to be implemented by the plugin to act(smart indent) accordingly.
              ...
              * @param event  The wxStyledTextCtrl event fired to react accordingly (see cbEditor::CreateEditor, namely scintilla_events)
              */
            virtual void Call(cbEditor* editor, wxStyledTextEvent& event) const;
